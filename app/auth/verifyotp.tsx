...
const VerifyOTP = () => {
  ...
  const onVerify = async () => {
    ...
    try {
      ...
      if (mode === 'recoverAccount') {
        ...
        await routePush(
          SCREENS.REGISTER,
          {isPassed: true, phoneNo: phoneNo as string, regNo: regNo as string},
          true,
        );
  ...
  return (
    <View className="flex-1 bg-bgPrimary" style={{paddingTop: insets.top}}>
      <Header title="" onBack={() => router.back()} bgColor="#24292D" />
      <CustomScrollView>
        <View className="mx-6 mt-10 flex-1 items-center">
          <Text className="text-xl font-bold text-white">Баталгаажуулах</Text>
          <Text className="mt-3 text-center text-sm text-white opacity-70">
          ...
          <Text className="mt-8 text-lg text-white opacity-70">
            {`0${Math.floor(time / 60)}:${time % 60 < 10 ? '0' : ''}${time % 60}`}
          </Text>
          <Text className="mt-2 text-sm text-[#6265FE]">Дахин илгээх үү?</Text>
